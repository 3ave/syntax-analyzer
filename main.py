...
def syntax_parse(symbols: list, index = 0) -> Tuple[Status, str]:
    ...
    while True:
        ...
        if (next_one not in GROUPE_TABLE[current].keys()):
            e = "symbol " + str(next_one) + ", line " + str(error_line)
            return (Status.ERROR, e)
        elif (GROUPE_TABLE[current][next_one][0] == 'S'):
            index += 1
            error_line += 1
            stack.append(int(GROUPE_TABLE[current][next_one][1:]))
        elif (GROUPE_TABLE[current][next_one][0] == 'R'):
            table_num = GROUPE_TABLE[current][next_one][1:]

            non_termi, termi = LINKED_GROUP[int(table_num)][:2]
            ...
            if GROUPE_TABLE[stack[-1]].get(non_termi) is None:
                error = "symbol " + str(next_one) + ", line " + str(error_line)
                return Status.ERROR, error

            # GOTO
            stack.append(GROUPE_TABLE[stack[-1]][non_termi])
            if termi != 'epsilon':
